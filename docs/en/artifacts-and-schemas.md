...
`meguribi promote owner/repo#123` atomically writes a Problem draft to `~/.local/share/meguribi/problems/<owner>/<repo>/from-issue-123/problem.json` and updates the source Issue marker comment. No new Issue is created unless `--create-issue` is explicitly confirmed.
...
The Problem draft generated by `promote` retains the source Hypothesis URL, observation evidence, and unconfirmed items. `targetUser`, `userImpact`, and `currentWorkaround` remain `null` when the input does not provide them; the command does not invent a solution.
...
```json
{
  ...
  "status": "draft",
  "humanApprovalRequired": true,
  "title": "Problem: Users cannot complete registration",
  "problem": "Some users cannot complete registration",
  "targetUser": null,
  "evidence": ["Drop-off was reported on the registration screen"],
  "userImpact": null,
  "currentWorkaround": null,
  "unconfirmedItems": ["Target user", "User impact", "Current workaround"],
...
- `minimumSupportedVersion`: the minimum supported Devin CLI version applied by the smoke.
...
- `warnings`: warnings from MCP policy or the adapter; also includes an `unknown` version warning.

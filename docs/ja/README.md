...
実装済みの delivery CLI は `doctor`、`run`、`resume` です。`doctor` は Devin / Cursor の ACP 実行可否を診断し、`run` と `resume` は実 GitHub / Git / Codex / Verifier と、明示的に選択した AgentAdapter をオーケストレーションします。成長ループ系コマンドは、現時点では仕様・土台の段階です。

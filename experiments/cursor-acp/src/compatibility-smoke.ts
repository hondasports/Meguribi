...
const DEFAULT_ARTIFACT_ROOT = path.resolve(__dirname, "../../../artifacts/cursor-acp");

export interface CursorCompatibilitySmokeResult {
  schemaVersion: 1;
  artifactType: "cursor-acp-compatibility-smoke";
  optIn: boolean;
  cliVersion: string;
...
function fixtureContext(fixture: Fixture): ImplementationContext {
  return {
    issue: {
      source: "cursor-acp-compatibility-smoke",
    ...
    primarySkill: "cursor-integration",
...
function deriveVersionWarnings(diagnosis: AgentDiagnosis): string[] {
  if (diagnosis.version.status === "unknown") {
    return [
      `Cursor CLI version could not be parsed; it may be untested against the minimum supported version ${MINIMUM_SUPPORTED_CURSOR_CLI_VERSION}.`,
    ];
  }
  if (diagnosis.version.status === "unsupported") {
    return [
      `Cursor CLI version ${diagnosis.version.raw ?? "unknown"} is below the minimum supported version ${MINIMUM_SUPPORTED_CURSOR_CLI_VERSION}.`,
    ];
  }
  return [];
}
...
): CursorCompatibilitySmokeResult {
...
function buildBaseResult(
  overrides: Partial<CursorCompatibilitySmokeResult> &
    Pick<
      CursorCompatibilitySmokeResult,
...
function buildBlockedResultFromDiagnosis(
  ...
  return buildBaseResult({
    ...
    error: diagnosis.errors[0]?.message ?? "Cursor CLI is not runnable",
  });
}
...
function redactResult(result: CursorCompatibilitySmokeResult): CursorCompatibilitySmokeResult {
  return redactJsonValue(result) as CursorCompatibilitySmokeResult;
}

async function writeAndReturnResult(
  directory: string,
  result: CursorCompatibilitySmokeResult,
): Promise<CursorCompatibilitySmokeResult> {
...
export async function runCompatibilitySmoke(
  ...
  const minimumSupportedVersion = MINIMUM_SUPPORTED_CURSOR_CLI_VERSION;
  const optIn = options.optIn ?? process.env.MEGURIBI_RUN_REAL_CURSOR_SMOKE === "1";
  ...
  if (!optIn && !options.fake) {
    const result = buildBaseResult({
      ...
      executedAt,
      warnings: [
        "Real Cursor smoke requires MEGURIBI_RUN_REAL_CURSOR_SMOKE=1; no external agent was started.",
      ],
    });
  ...
  let diagnosis: AgentDiagnosis | undefined;
  ...
  try {
    ...
    const adapter = createCursorAcpAdapter({
      executable,
      executableArgs,
      acpArgs,
...
const REAL_SMOKE_WARNING = `WARNING: This command runs the real Cursor CLI against a temporary fixture.
- It may incur billing and network usage.
- It inherits the current user environment, including any saved Cursor credentials.
- MCP isolation is not mechanically guaranteed; inherited MCP is denied by policy, but the smoke cannot fully prevent Cursor from trying.
`;
...
export async function runCompatibilitySmokeCli(): Promise<void> {
  ...
  const optIn = process.env.MEGURIBI_RUN_REAL_CURSOR_SMOKE === "1";
  ...
  const result = await runCompatibilitySmoke({
    fake,
    optIn: fake || optIn,
    fakeMode: process.env.MEGURIBI_FAKE_CURSOR_SCENARIO ?? "write-in-scope",
  });

...
export async function createFixture(prefix = path.join(os.tmpdir(), "meguribi-cursor-acp-")): Promise<Fixture> {
  ...
  await runCommand("git", ["worktree", "add", "-b", "issue-cursor-smoke-fixture", worktree], normalCheckout);
  return {
    root,
    normalCheckout,
    worktree,

...
export const CURSOR_PROMPT_VERSION = "meguribi-cursor-prompt/v1";
const DEFAULT_MAX_PROMPT_CHARS = 48_000;

export interface BuiltCursorPrompt {
  version: string;
  hash: string;
  content: string;
}

export class CursorPromptBuildError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "CursorPromptBuildError";
  }
}
...
function canonicalAllowedPaths(worktreePath: string, allowedPaths: readonly string[]): string[] {
  ...
  return allowedPaths.map((candidate) => {
    ...
    if (!isWithin(root, absolute)) {
      throw new CursorPromptBuildError(`Allowed path escapes the assigned worktree: ${candidate}`);
    }
...
export function buildCursorPrompt(context: ImplementationContext): BuiltCursorPrompt {
  v.parse(ImplementationContextSchema, context);
  const maxChars = context.limits.maxPromptChars || DEFAULT_MAX_PROMPT_CHARS;
  if (!Number.isSafeInteger(maxChars) || maxChars < 1) {
    throw new CursorPromptBuildError("maxPromptChars must be a positive integer");
  }
  ...
  if (content.length > maxChars) {
    throw new CursorPromptBuildError(`Cursor prompt exceeds maxPromptChars (${maxChars})`);
  }
  return {
    version: CURSOR_PROMPT_VERSION,

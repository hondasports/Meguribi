 ...
 */
export function formatCursorDiagnosisHuman(diagnosis: AgentDiagnosis): string {
  const lines: string[] = [];

  if (diagnosis.executable.status === "missing") {
    lines.push("✗ Cursor CLI: missing");
    ...
    const mark =
      diagnosis.version.status === "unsupported"
        ...
        : diagnosis.version.status === "unknown"
          ? "!"
          : "✓";
    lines.push(`${mark} Cursor CLI: ${sanitizeDiagnosticDisplayText(diagnosis.version.raw)}`);
  } else {
    lines.push("! Cursor CLI: version unknown");
  }

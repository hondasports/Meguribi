...
export interface StartAcpInput {
  executable: string;
  /**
...
export interface AcpConnection {
  ...
  prompt(input: { content: string }): AsyncIterable<RawAcpEvent>;
  cancel(): Promise<void>;
...
export interface AcpTransport {
  start(input: StartAcpInput): Promise<AcpConnection>;
}
...
export class AcpProcessLifecycle {
  ...
  private exitError: AcpTransportError | undefined;
  private readonly listeners = new Set<(error: AcpTransportError) => void>();
  ...
  constructor(processHandle: Pick<ManagedProcess, "waitForExit">) {
    void processHandle.waitForExit().then(
      ...
      (error: unknown) => {
        this.recordUnexpected(toAcpTransportError(error, "spawn_failure"));
      },
    );
  }
  ...
  get unexpectedError(): AcpTransportError | undefined {
  ...
  onUnexpectedExit(listener: (error: AcpTransportError) => void): () => void {
    if (this.unexpectedError) {
  ...
  private recordUnexpected(error: AcpTransportError): void {
...
class AcpConnectionImpl implements AcpConnection {
  ...
  async *prompt(input: { content: string }): AsyncIterable<RawAcpEvent> {
    ...
    if (preflight && preflight.outcome !== "allow") {
      throw new AcpTransportError("policy_blocked", preflight.reason);
    }

    const queue: Array<RawAcpEvent | { kind: "error"; error: AcpTransportError }> = [];
    let wake: (() => void) | undefined;
    let settled = false;
    let stopReason: string | undefined;

    const push = (event: RawAcpEvent | { kind: "error"; error: AcpTransportError }) => {
      queue.push(event);
      wake?.();
    };
    ...
    const unsubscribeMcp = this.mcpMonitor?.onDecision((decision) => {
      if (decision.outcome !== "allow") {
        push({ kind: "error", error: new AcpTransportError("policy_blocked", decision.reason) });
      }
    });
    ...
    const promptPromise = withTimeout(promptCall, this.promptTimeoutMs, "ACP prompt", "turn_timeout")
      ...
      .catch((error: unknown) => {
        settled = true;
        push({ kind: "error", error: toAcpTransportError(error, "prompt_send_failure") });
        wake?.();
      });
    ...
    try {
      ...
      throw toAcpTransportError(error, "prompt_send_failure");
    } finally {
      unsubscribeExit();
      unsubscribeMcp?.();
  ...
  async cancel(): Promise<void> {
    try {
      ...
      throw toAcpTransportError(error, "cancelled");
    }
  }
...
export class AcpTransportImpl implements AcpTransport {
  async start(input: StartAcpInput): Promise<AcpConnection> {
    try {
      input.assertRunnable(input.diagnosis);
    } catch (error) {
      throw new AcpTransportError(
        "not_runnable",
        error instanceof Error ? error.message : "Agent diagnosis is not runnable",
      );
    }
    ...
    try {
      ...
      throw toAcpTransportError(error, "spawn_failure");
    }
    ...
    try {
      ...
      throw toAcpTransportError(error, "initialize_failure");
    }
  }
}

export function createAcpTransport(): AcpTransport {
  return new AcpTransportImpl();
}

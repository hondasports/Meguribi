/**
 * Devin CLI の version 文字列を解析する。
 */

const SEMVER_LIKE =
  /(?:^|\s)v?(\d+)\.(\d+)\.(\d+)(?:[-+][0-9A-Za-z.-]+)?(?:\s|$)/;

export interface ParsedDevinVersion {
  raw: string;
  major?: number;
  minor?: number;
  patch?: number;
...
export function parseDevinVersionOutput(stdout: string): ParsedDevinVersion {
...
export function parseMinimumVersion(
  ...
  const parsed = parseDevinVersionOutput(minimum);
  if (
    !parsed.parseable ||
